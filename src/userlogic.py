...
the allowable value of RAM or CPU report it in stdout and logs.
'''
...
class UserLogic(object):
    __help = ('Specify valid arguments: \n'
              ...
              '     poll [seconds] - polling time of processes usage. Default is 60 seconds. Boundaries from 5 to 1000 seconds. \n'
              ' \n'
              '# rdoverload disable \n'
              ' Unload and delete daemon from autostart. '
              )

    __default_enable_settings = {"cpu": 20, "ram": 20, "poll": 60}
    __pid_file = '/root/overload.pid'
    ...
    def execute(self, args):
        ...
        if args_len == 1 and args[0] == "disable":
            ProcessLogic.disable()
        elif args[0] == "enable":
            settings = self.__parse_enable_args(args[1:])
            ProcessLogic(settings, self.__pid_file).enable()
        else:
            self.__help_and_exit()
    ...
    def __is_setting_valid(self, key, value):
        ...
        self.__help_and_exit("Wrong key/value for [" + key + "=" + str(value) + "]\n")
